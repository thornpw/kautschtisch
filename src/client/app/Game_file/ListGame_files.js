...
export default React.createClass({
  ...
  load_number_of_game_files() {
    $.ajax({
      url: encodeURI(sprintf('http://localhost:3300/api/game_file/count/%s',this.get_filter())),
      dataType: 'json',
      cache: false,
  ...
  load_game_files: function() {
    $.ajax({
      url: sprintf("http://localhost:3300/api/game_file/%s/%s/%s",get_game_file_offset(),get_game_file_limit(),this.get_filter()),
      dataType: 'json',
      cache: false,
      success: function(data) {
