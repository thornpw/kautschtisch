...
// Organisation List
// -----------------------------------------------------------------------------
class ListOrganisations extends React.Component {
  constructor(props) {
    super(props)

    this.object_type='organisations'
    this.state = {
      data: []
    }
  ...
  load_data() {
    $.ajax({
      url: sprintf("http://localhost:3300/api/db/search/KOrganisation?offset=%s&limit=%s%s",this.redux_state.offset,this.redux_state.limit,buildConditionFromFilters(this.redux_state.filters,this.redux_state.filter_columns)),
      dataType: 'json',
      cache: false,
  ...
  createOrganisation(){
      ...
      for(var i = 0; i < items.length; i++) {
        output.push(<Organisation key={i} data={items[i]} update_parent_data={this.load_data.bind(this)}/>);
      }
      return output;
  }
    ...
    return (
      <div>
        <Panel header={"Organisations"}>
          ...
          >
            <ListColumn object_type='organisations' column_number={0} display_name="Name" update_data={this.load_data.bind(this)}/>
          </ObjectList>
        </Panel>
      </div>
    )
